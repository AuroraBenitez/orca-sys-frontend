...
app.controller('cambiarContrasenhaController', function($scope, $location, $rootScope, $cookies, $dialogs, UsuariosService, $cookies,$timeout) {
    $scope.datos = {};
    ...
    $scope.modificar = function() {
        if( $scope.datos.contrasenha!= null && typeof $scope.datos.contrasenha != undefined
            && $scope.datos.nuevaContrasenha!= null && typeof $scope.datos.nuevaContrasenha !=undefined
            && $scope.datos.contrasenha!= $scope.datos.nuevaContrasenha ){
            dlg = $dialogs.create('/dialogs/error.html', 'errorDialogController' ,{msg:'Error. Las contrase&ntilde;as son diferentes'},{key: false,back: 'static'});
        } else{
    ...
    $scope.listarUsuarios= function(){
        var json = angular.toJson({"usuario":$scope.datos.usuario});
        UsuariosService.listarJson(json).then(function(response){
            if(response.status == 200){
                $scope.listaUsuarios = response.data;
                $scope.datos.codigo = $scope.listaUsuarios[0].codigo;
            }else{
                dlg = $dialogs.create('/dialogs/error.html', 'errorDialogController' ,{msg:'Error de Sistema, consulte con el administrador'},{key: false,back: 'static'});
            }
        })
